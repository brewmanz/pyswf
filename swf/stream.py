...
class SWFStream(object):
    ...
    def __init__(self, theFile):
        ...
        self.f = theFile
        self._allBytesRead = list() # keep tally of everything read
    ...
    def readbits(self, nBits):
        ...
        if nBits % 8 == 0 and self._bits_pending == 0:
            return self._read_bytes_aligned(nBits // 8)
        ...
        while nBits > 0:
            if self._bits_pending > 0:
                ...
                take = min(self._bits_pending, nBits)
            ...
            if r == b'':
                raise EOFError
            self._allBytesRead.extend(r)
            self._partial_byte = ord(r)
            self._bits_pending = 8

        return out

    def readFB(self, nBits):
        """ Read a float using the specified number of bits """
        return float(self.readSB(nBits)) / 65536.0

    def readSB(self, nBits):
        """ Read a signed int using the specified number of bits """
        shift = 32 - nBits
        return int32(self.readbits(nBits) << shift) >> shift

    def readUB(self, nBits):
        """ Read a unsigned int using the specified number of bits """
        return self.readbits(nBits)

    def readSI8(self):
        """ Read a signed byte """
        self.reset_bits_pending();
        buf = self.f.read(1)
        self._allBytesRead.extend(buf)
    ...
    def read(self, count=0):
        """ Read """
        buf = self.f.read(count) if count > 0 else self.f.read()
        self._allBytesRead.extend(buf)
        return buf
